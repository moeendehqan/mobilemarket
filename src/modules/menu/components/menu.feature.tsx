...
import { BiHomeAlt, BiUser, BiPackage, BiCart, BiCog, BiInfoCircle, BiPhone, BiChevronLeft, BiMenu } from "react-icons/bi";
...
const MenuFeature = ({ isOpen = true, onToggle }: MenuFeatureProps) => {
    const { data: user, permissionsCheck } = useUser();
    ...
    return (
        <>
            ...
            <div className={`fixed top-0 right-0 h-full z-50 ${!isOpen ? 'lg:block hidden' : ''}`}>
                ...
                >
                    <BiChevronLeft className={`transform transition-transform duration-200 ${isOpen ? 'rotate-180' : ''} text-gray-600`} />
                </button>
                    ...
                    <nav className="space-y-2">
                        {data.filter(item => item.is_active).map((item) => (
                            ...
                            >
                                ...
                                {!isOpen && (
                                    <div className="absolute right-14 bg-gray-800 text-white px-2 py-1 rounded text-sm opacity-0 invisible
